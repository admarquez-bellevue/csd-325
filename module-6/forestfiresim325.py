...
def createNewForest():
    ...
    # Define the new lake feature in the center of the forest.
    lake_center_x = WIDTH // 2
    lake_center_y = HEIGHT // 2
    lake_radius = min(WIDTH, HEIGHT) // 4
    
    for x in range(WIDTH):
        for y in range(HEIGHT):
            # Randomized lake  shape
            distance_from_center = ((x - lake_center_x) ** 2 + (y - lake_center_y) ** 2) ** 0.5
            lake_threshold = lake_radius + random.uniform(-2, 2)
            if distance_from_center <= lake_threshold:
                forest[(x, y)] = WATER
